...
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// 로그인 Service 호출 후 결과 반환 받기 _로그인: 정보를 기록
			...
			*/
			...
			/*
			 foward :  카페 > 커피주문 > 캐셔 > 바리스타   // 같은 곳
			 redirect : 카페 > 김밥주문 > 캐셔 > 옆집으로 가세요 > 김밥집 > 다시 김밥주문 // 다른 곳
			
			 *** forward / redirect 반드시 구분해야 함!!!
			...
			***** redirect(재요청) *****
			...
			// 메인 페이지로 redirect
			// -> 메인 페이지를 요청한 것이기 때문에 
			//    주소창에 주소가 메인 페이지 주소로 변함.    cf. forward : 메인페이지 메인화면 위에 정보가 출력됨(주소창 주소가 안 변함).
												//     redirect는 새로 메인페이지를 불러옴(주소창 주소가 변함).
			 ...
			 *** forward / redirect 차이점 ***
			 ...
			 forward
			 - 주소창 변화 X  (localhost -> localhost/member/login  // 같은 창에서 실행)
			 - JSP 경로 작성
			 - req, resp가 유지된다.
			 
			 redirect
			 - 주소창 변화 O (localhost -> localhost   : / -> /member/login -> /)
			 - 요청 주소 작성
			 - req, resp가 유지되지 않는다.
...
//			req.setAttribute("loginMember", loginMember);					
			...
			if(loginMember != null) { // 로그인 성공 시
				...
				session.setMaxInactiveInterval(60 * 60); // 60초 * 60분 = 1시간(3600초) //만료시간 지정 안하면 무제한
...
/*				------------------------------------------------------------------------
				...
				//   클라이언트 컴퓨터에 파일 형태로 저장됨.   // Cookie key, value 형태
			    ...
			    if(req.getParameter("saveId") != null) { //체크된 경우
			    	...
			    	cookie.setMaxAge(60 * 60 * 24 * 30); // 30일
			    	 ...
			    	 // 4) 쿠키의 유효기간을 0초로 설정
			    	 // == 클라이언트에 저장된 savdId 쿠키를 삭제하라는 의미
