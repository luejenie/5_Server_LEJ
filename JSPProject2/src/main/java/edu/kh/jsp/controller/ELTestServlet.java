...
public class ELTestServlet extends HttpServlet{
	...
	//  __a태그(get방식)와 form태그(post방식)가 같은 주소를 사용하는데도 둘다 쓸 수 있는 이유
	//  __ GET(SELECT) / POST(INSERT) / PUT(UPDATE) / DELETE(DELETE) _ openAPI 사용할 때
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/el/elTest.jsp");
												...
												// 단, 내부 요청(Servlet, JSP에서의 직접 접근)은 가능. _서블릿, JSP에서 경로 쓰는 것.
...
//		System.out.println(req.getParameter("inputAddress"));
		...
		// 요청 위임 시 추가할 값 세팅
		req.setAttribute("message", message);  //key, value
